...
function nectar_button($atts, $content = null) {  
    extract(shortcode_atts(array("size" => 'small', "url" => '#', 'color' => 'Accent-Color', 'color_override' => '', "text" => 'Button Text', 'image' => '', 'open_new_tab' => '0'), $atts));
	...
	switch ($size) {
		case 'small' :
			$button_open_tag = '<a class="nectar-button small '. strtolower($color) . $has_icon .'" '. $target;
			break;
		case 'medium' :
			$button_open_tag = '<a class="nectar-button medium ' . strtolower($color) . $has_icon .'" '. $target;
			break;
		case 'large' :
			$button_open_tag = '<a class="nectar-button large '. strtolower($color) . $has_icon .'" '. $target;
			break;	
	}
	
	$color_or = (!empty($color_override)) ? 'data-color-override="'. $color_override.'"' : 'data-color-override="false"';	
    return $button_open_tag . ' href="' . $url . '" '.$color_or.'><span>' . $text . '</span> ' . $button_icon . '</a>';
}
...
function nectar_portfolio_processing($atts, $content = null) {
	extract(shortcode_atts(array("layout" => '3', 'category' => 'all', 'project_style' => '1', 'starting_category' => '', 'masonry_style' => '0', 'enable_sortable' => '0', 'constrain_max_cols' => 'false', 'horizontal_filters' => '0','lightbox_only' => '0', 'enable_pagination' => '0', 'projects_per_page' => '-1'), $atts));   
	...
	<div class="portfolio-wrap <?php if($project_style == '1' && $span_num == 'elastic-portfolio-item') echo 'default-style'; ?>">
			...
			<div id="portfolio" class="row portfolio-items <?php if($masonry_layout == 'true') echo 'masonry-items'; else { echo 'no-masonry'; } ?> <?php echo $constrain_col_class; ?>" data-starting-filter="<?php echo $starting_category; ?>" data-categories-to-show="<?php echo $category; ?>" data-col-num="<?php echo $cols; ?>">
				<?php 
							...
							<div class="flex-gallery"> 
									 <ul class="slides">
										...
										foreach( $gallery_ids as $image_id ) {
										     echo '<li>' . wp_get_attachment_image($image_id, 'portfolio-thumb', false, $attr) . '</li>';
										} ?>
							    	</ul>
						   	 </div><!--/gallery-->
