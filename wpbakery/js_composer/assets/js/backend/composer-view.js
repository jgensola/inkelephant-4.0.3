...
(function ($) {
    ...
    var ShortcodeView = vc.shortcode_view = Backbone.View.extend({
        ...
        addElement:function (e) {     	
            ...
            $('.wpb-content-layouts li').each(function(i){
        ...
        render:function () {
            ...
            if (tag !== false && tag !== false && !_.isUndefined(vc.map[tag].allowed_container_element)) {
                if (vc.map[tag].allowed_container_element === false) {
                    item_selector += ':not([data-is-container=true][data-element!=clients][data-element!=testimonial_slider],[data-element=nectar_portfolio])';
		             ...
		             $.each(this.model.collection.models,function(k,v){
		            	...
		            	if(v.attributes.id == $rootId || v.attributes.id == $parentId) {
		            		if(v.attributes.params.type == 'full_width_content') {
		            			...
		            			$('select.column_padding').parents('.wpb_el_type_dropdown').show();
		            			...
		            			$('select.column_padding').parents('.wpb_el_type_dropdown').hide();
        ...
        render:function () {
            ...
            if (this.accessPolicy !== 'only') {
            	...
            	//portfolio extra
            		  ...
            		  this.$switchButton = $('<a class="wpb_switch-to-composer button-primary" href="#">' + window.i18nLocale.main_button_title + '</a>').prependTo('#normal-sortables').wrap('<p class="composer-switch" />');        		
            	}
            ...
            if (confirm((window.i18nLocale.are_you_sure_convert_to_new_version)))
                $.ajax({
                    ...
                    context:this
                        ...
                        //fix multi rows of columns inside of FWS
			            $('.wpb_vc_row .vc_row-fluid').each(function(){
			            	$(this).find(' > .wpb_vc_column').each(function(){
			            		if($(this).position().left < 15) $(this).css('margin-left',0);
			            	});
